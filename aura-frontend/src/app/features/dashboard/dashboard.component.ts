...
import { DashboardDto, MetodoPagoDto, ProductoStockBajoDto, VentasSemanaDto } from '../../core/models/dashboard.model';
...
export class DashboardComponent implements OnInit {
  ...
  private async loadChartVentasSemana(): Promise<void> {
    try {
      const response = await lastValueFrom(this.dashboardService.ventasSemana());
  ...
  private async loadChartMetodoPago(): Promise<void> {
    try {
      const response = await lastValueFrom(this.dashboardService.ventasMetodoPago());
        ...
        datasets: [
          {
            ...
            backgroundColor: data.map(d => colores[d.metodo_pago] ?? '#9CA3AF'),
            borderWidth: 2,
            borderColor: '#FFFFFF',
            hoverOffset: 6,
          },
