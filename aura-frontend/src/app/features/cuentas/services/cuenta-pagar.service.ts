...
export class CuentaPagarService {
  ...
  createAbono(
    cuentaId: number,
    dto: CreateAbonoPagarDto,
  ): Observable<{
    status: number;
    message: string;
    error: boolean;
    data: AbonoPagarModel;
  }> {
  ...
  getResumen(
    ...
    if (params?.proveedorId)
      queryParams.set('proveedorId', String(params.proveedorId));
    if (params?.params?.estado) queryParams.set('estado', params.params.estado);
