...
@Component({
  ...
  templateUrl: './index-recaudos.component.html',
  styleUrls: ['./index-recaudos.component.scss'],
})
export class IndexRecaudosComponent implements OnInit {
  ...
  readonly categoriasOpts = CATEGORIAS_RECAUDO;
  ...
  async cargar(): Promise<void> {
    ...
    try {
      ...
      const res = await lastValueFrom(
        this.service.listarRecaudos(desde, hasta, this.cuentaFiltroId),
      );
      this.movimientos = res?.data ?? [];
    } catch {
      this.alertService.showError('Error', 'No se pudieron cargar los recaudos');
    } finally {
      this.loading = false;
      this.cdr.markForCheck();
    }
  ...
  async guardar(): Promise<void> {
    ...
    this.cdr.markForCheck();
    try {
      await lastValueFrom(this.service.crearRecaudo(this.form));
      this.alertService.showSuccess('Recaudo registrado', '');
  ...
  async anular(m: TesoreriaMovimientoModel): Promise<void> {
    try {
      ...
      this.alertService.showSuccess('Recaudo anulado', '');
      this.cargar();
    } catch {
